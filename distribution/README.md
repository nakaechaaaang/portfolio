...
サイトを「パスワードを知っている人だけ」に限定したい場合は、`template/auth.js` の
パスワードを設定するだけでログイン画面が有効になります（**既定はOFF＝誰でも閲覧可**）。

```js
const SITE_PASSWORD = ''; // ← 好きなパスワードを入れると有効。空ならロックなし
```

> ⚠️ これは静的サイト向けの「簡易ロック」です。画面は隠せますが、ページのソースを
> 見れば中身は取得できるため、**完全な機密保護にはなりません**（限定共有など軽い用途向け）。
> 本格的に保護したい場合は Cloudflare Access などのサーバー側認証を検討してください。
